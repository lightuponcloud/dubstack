...
class LockTest(unittest.TestCase):
    ...
    #         if obj['object_key'] == object_key:
    #             print(obj['version'])
    #             last_seen_version = b64decode(obj["version"]).decode()
    ...
    #     dir_name_dst_prefix = encode_to_hex(dir_name)
    #     self.client.create_pseudo_directory(TEST_BUCKET_1, dir_name)
    #
    #     result = self.client.upload(TEST_BUCKET_1, fn)
    #     object_key = result['object_key']
    #
    #     # 2. lock it and check for "is_locked": True
    #     response = self.client.patch(TEST_BUCKET_1, "lock", [object_key])
    #     result = response.json()
    #     # print(response.content.decode())
    #     self.assertEqual(result[0]['is_locked'], "true")
    #     self.assertEqual(response.status_code, 200)
    #
    #     # 3.1 Try to replace locked file by move operation
    #     src_object_keys = [object_key]
    #     # print(src_object_keys)
    #     response = self.client.move(TEST_BUCKET_1, TEST_BUCKET_1, src_object_keys, dst_prefix=dir_name_dst_prefix)
    #     # print(response.content.decode())
    ...
    #     self.assertEqual(response.status_code, 200)
    #
    #     response = self.client.delete(TEST_BUCKET_1, [dir_name_dst_prefix])
    #     # print(response.content.decode())
    #     self.assertEqual(response.status_code, 200)
